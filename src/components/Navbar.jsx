  ...
  return (
    <div className='flex justify-center border-none fixed z-50 top-0 left-0 right-0'>
      ...
      <div className="flex items-center justify-between md:w-1/3 w-10/12 relative">
        <div>
        <Link to="/movie-site/" className={`font-bold md:text-2xl text-base ${currentPath === '/movie-site/' || currentPath.startsWith('/movie-site/Films') ? 'text-black' : 'text-gray-400'}`}>
  Films
        </Link>
        </div>
        <div>
          <Link to="/movie-site/Serial" className={`font-bold md:text-2xl text-base ${getLinkClass('/movie-site/Serial')}`}>
            Serial
          </Link>
        </div>
        ...
        <div>
          <Link to="/movie-site/Kids" className={`font-bold md:text-2xl text-base ${getLinkClass('/movie-site/Kids')}`}>
            Kids
          </Link>
        </div>
      </div>
